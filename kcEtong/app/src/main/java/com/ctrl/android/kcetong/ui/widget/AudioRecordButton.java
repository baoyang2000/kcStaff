package com.ctrl.android.kcetong.ui.widget;
...
import com.ctrl.android.kcetong.toolkit.util.AudioFileFunc;
import com.ctrl.android.kcetong.toolkit.util.AudioManager;
import com.ctrl.android.kcetong.toolkit.util.AudioRecordFunc;
...
public class AudioRecordButton extends Button implements AudioRecordFunc.AudioStageListener {
...
//		mAudioManager.setHandle(mp3handler);
		...
		audioRecordFunc.setOnAudioStageListener(this);
		audioRecordFunc.setHandle(mp3handler);
...
//					File file = new File(mAudioManager.getCurrentFilePath());
					...
					if (FileSaveUtil.isFileExists(file)) {
						mListener.onFinished(mTime, AudioFileFunc.getWavFilePath());
					}else{
...
//						mListener.onFinished(f1,mAudioManager.getCurrentFilePath());
						mListener.onFinished(f1,AudioFileFunc.getWavFilePath());
					}else{
						mp3handler.sendEmptyMessage(AudioManager.MSG_ERROR_AUDIO_RECORD);
					}
				}
